...
MaterialAdded = 'water_april_15'  # Material added to water
...
excel_path = File_location_excel + str(Date) + '_' + str(dateN.strftime("%H")) + '-' + str(
    dateN.strftime("%M")) + '-' + str(dateN.strftime("%S")) + '_' + MaterialAdded + '.xlsx'

# Create path to Plots
plot_path = File_Location_plots + str(Date) + '_' + str(dateN.strftime("%H")) + '-' + str(
    dateN.strftime("%M")) + '-' + str(dateN.strftime("%S")) + '_' + MaterialAdded + '.png'
...
table = pd.DataFrame(0, range(meas_amount + 1), range(18))
table = table.astype('float32')
table.rename(
    columns={0: 'Date [dd-mm-yy]', 1: 'Time [hh-mm-ss]', 2: 'Measurement [-]', 3: 'ABS210 [AU]', 4: 'ABS254 [AU]',
             5: 'SAC254 [AU]', 6: 'UVT254 [AU]', 7: 'ABS360 [AU]', 8: 'Turbidity [FNU]', 9: 'Chloor [ppm]',
             10: 'Temperature [C]', 11: 'Acidity [pH]', 12: 'Conductivity [uS/cm]', 13: 'TRP[ug/l]', 14: 'BODeq',
             15: 'CODeq', 16: 'NO2', 17: 'NO3'}, inplace=True)
...
while N_measurements <= meas_amount:
    ...
    if timeSeconds == 0:  # Checks if seconds are equal to zero
        ...
        table.iloc[N_measurements, 15] = CODeq[0]
        table.iloc[N_measurements, 16] = NO2[0]
        table.iloc[N_measurements, 17] = NO3[0]
        # new sensor can be added here
